...
//сам лабиринт на плоскости, если элемент= 0-стена, если =1-проход(двумерный массив, 
//сделал динамическим чтобы при запуске можно было задавать размеры)

int** maz;
int cons_x, cons_y;//переменные для размера лабиринта, чем больше тем дольше генерируется
vector<int> points_x;//глупое решение, но первое что пришло в голову, хранить в 2х массивах 
										//координаты тех точек лабиринта, которые уже вырублены, т.е вершины   
vector<int> points_y;//чтобы из них потом продолжать строить ответвления,
											// рандомно выбирая точки из этих массивов и рандомное направление прокладки пути 
...
Maze::Maze(int y, int x) {//int y = 25, int x = 80
...
//функция для проверки вершин, если из текущей вершины невозможно ни в одну сторону прорубить проход,
//то эта вершина удаляется из списка points, чтобы рандом больше ее не выбирал
...
void Maze::draw() {//отрисовка лабиринта 
